...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Number of independent columns(rows) of $A$ $\Rightarrow$ $\mathrm{rank}(A^{\top}) = \mathrm{rank}(A)$
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item $A^{\top}\mathbf{c}$ $\Rightarrow$ $\mathbf{c} = \mathbf{0}$
        \item $\det(AA^{\top}) \neq 0$
        \item $A$ has right inverse: $\exists B$ s.t. $AB = I$ with $B = A^{\top}(AA^{\top})^{-1}$
    \end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item $A \in \mathbb{S}^n$ can be decomposed to $A = U\Lambda U^{\top}$, i.e. $AU = U\Lambda$
        \item $\Lambda$: Diagonal matrix with $\lambda_i$ at $i$-th diagonal,
            $U$: Orthogonal matrix $\Rightarrow$ $U^{\top} = U^{-1}$
    \end{itemize}
    \item Positive Definiteness of Symmetric Matrices
    \begin{itemize}
        \item $A = U\Lambda U^{\top} = \sum_i \lambda_i \mathbf{u}_i \mathbf{u}_i^{\top}$: Sum of rank-1 matrices
        \item $\mathbf{u}_i$: bases $\Rightarrow$ $\mathbf{x} = \sum_{i} \hat{\mathbf{x}_i}\mathbf{u}_i$
            ...
            $\Rightarrow$ $\mathbf{x}^{\top}A\mathbf{x} = \sum_{i} \lambda_i \hat{\mathbf{x}_i}^2$
...
\subsubsection*{Definition}
\begin{itemize}
    \item A function that maps length $n$ vector to scalar s.t. $f(x) = \mathbf{x}^{\top}Q\mathbf{x}$ ($Q \in \mathbb{S}^n$)
    \item $Q \notin \mathbb{S}^n$ can be replaced by $\tilde{Q} = \frac 1 2 \left(Q + Q^{\top} \right) \in \mathbb{S}^n$,
...
\begin{itemize}
    ...
    \begin{itemize}
        \item Positive Definite (PD) Matrix:
            $ \forall \mathbf{x} \in \mathbb{R}^n \setminus \{\mathbf{0}\},~ \mathbf{x}^{\top}Q\mathbf{x} > 0 \Rightarrow Q \succ 0$
        \item Positive Semi-definite (PSD) Matrix:
            $ \forall \mathbf{x} \in \mathbb{R}^n \setminus \{\mathbf{0}\},~ \mathbf{x}^{\top}Q\mathbf{x} \geq 0 \Rightarrow Q \succeq 0$
        \item Negative Definite Matrix:
            $ \forall \mathbf{x} \in \mathbb{R}^n \setminus \{\mathbf{0}\},~ \mathbf{x}^{\top}Q\mathbf{x} < 0 \Rightarrow Q \prec 0$
        \item Negative Semi-definite Matrix:
            $ \forall \mathbf{x} \in \mathbb{R}^n \setminus \{\mathbf{0}\},~ \mathbf{x}^{\top}Q\mathbf{x} \leq 0 \Rightarrow Q \preceq 0$
        \item Indefinite Matrix: Sign of $\mathbf{x}^{\top}Q\mathbf{x}$ differs by $\mathbf{x}$
    \end{itemize}
    \item Notations
    \begin{itemize}
...
\begin{itemize}
    \item $\mathbf{x}^{\top} Q\mathbf{x} = \mathbf{x}^{\top} (U\Lambda U^{\top}) \mathbf{x}
        = \mathbf{x}^{\top} \left(\sum_i \lambda_i \mathbf{u}_i \mathbf{u}_i^{\top}\right) \mathbf{x}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item $\exists A^{-1}$, $A^{-1} \succ 0$ ($A^{-1} = QD^{-1}Q^{\top}$, Eigenvalues are $\lambda_i^{-1}$)
        \item $A + B \succ 0$
    \end{itemize}
    \item $A \in \mathbb{R}^{m \times n}$
    \begin{itemize}
        \item $\mathbf{x}^{\top}\left(A^{\top}A\right)\mathbf{x} = (A\mathbf{x})^{\top}(A\mathbf{x}) = \Vert A\mathbf{x} \Vert^2 \geq 0$
        \item $A^{\top}A \succeq 0$ ($\mathrm{rank}(A) = n~\Rightarrow~A^{\top}A \succ 0$)
        \item $AA^{\top} \succeq 0$ ($\mathrm{rank}(A) = m~\Rightarrow~AA^{\top} \succ 0$)
    \end{itemize}
    \item $\mathbf{v}\mathbf{v}^{\top} \in \mathbb{S}_+^n$: Rank-1 matrix
...
\begin{itemize}
    \item $\left\{\mathbf{x}~|~\mathbf{x}^{\top}Q\mathbf{x}=c,~Q \succ 0\right\}$ is an ellipse
    \item If $Q = I$, the given set is a sphere
    \item $1 = \mathbf{x}^{\top} Q\mathbf{x} = \sum_i \lambda_i y_i^2$ $\Rightarrow$
...
\begin{itemize}
    \item Definition: $R(Q, \mathbf{x}) = \frac {\mathbf{x}^{\top} Q\mathbf{x}} {\mathbf{x}^{\top}\mathbf{x}}$ for $Q \in \mathbb{S}^n$
    \item $\mathbf{x}$: unit vector $\Rightarrow$ $R(Q, \mathbf{x}) = \mathbf{x}^{\top}Q\mathbf{x}$
    \item $Q \succ 0$ $\Rightarrow$ Eigenvector with maximum(minimum) eigenvalue maximizes(minimizes) $R(Q, \mathbf{x})$
    \item Proof: $\max_{\mathbf{x} \neq \mathbf{0}} \frac {\mathbf{x}^{\top} Q\mathbf{x}} {\mathbf{x}^{\top}\mathbf{x}}
