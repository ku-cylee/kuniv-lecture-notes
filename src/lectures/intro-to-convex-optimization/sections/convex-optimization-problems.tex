...
\begin{equation}\begin{aligned}
    \mathrm{minimize}~~&~~f_0(\mathbf{x}) \\
    \mathrm{subject~to}~~&~~f_i(\mathbf{x}) \leq 0,~i=1,2,\cdots,m \\
        &~~\mathbf{a}_i^{\top} \mathbf{x} = b_i,~i=1,2,\cdots,p
\end{aligned}\end{equation}
\begin{itemize}
    \item $f_i$: convex
    \item Equality constraints: affine
    \item $\mathbf{a}_i^{\top} \mathbf{x} = b_i$ can be simply written in $A\mathbf{x} = \mathbf{b}$
...
\begin{itemize}
    ...
    \begin{equation}
        \forall~\mathbf{x} \in \mathcal{F},~\nabla f_0 \left(\mathbf{x}^{\ast}\right)^{\top} \left( \mathbf{x} - \mathbf{x}^{\ast} \right) \geq 0
    \end{equation}
    ...
    \begin{itemize}
        \item $\mathbf{x}^{\ast} \in \mathcal{F}$: optimal $\Leftrightarrow$
            $\nabla f_0 \left(\mathbf{x}^{\ast}\right) + A^{\top} \nu = 0$
...
\begin{equation}\begin{aligned}
    \mathrm{minimize}~~&~~f_0(\mathbf{x}) \\
    \mathrm{subject~to}~~&~~\mathbf{a}_i^{\top} \mathbf{x} \leq b_i~~(i \in \mathbb{Z} \cap [1,m]) \\ {}
\end{aligned}~~~\Leftrightarrow~~~\begin{aligned}
    \mathrm{minimize}~~&~~f_0(\mathbf{x}) \\
    \mathrm{subject~to}~~&~~\mathbf{a}_i^{\top} \mathbf{x} + s_i \leq b_i~~(i \in \mathbb{Z} \cap [1,m]) \\
...
\begin{equation}\begin{aligned}
    \mathrm{minimize}~~&~~\mathbf{c}^{\top} \mathbf{x} + d \\
...
\begin{itemize}
    ...
    \begin{equation}\begin{aligned}
        \mathrm{minimize}~~&~~\mathbf{c}^{\top} \mathbf{x} \\
    ...
    \begin{equation}\begin{aligned}
        \mathrm{minimize}~~&~~\max_{i \in \mathbb{Z} \cap [1,m]} \left\{ \mathbf{a}_i^{\top}\mathbf{x} + \mathbf{b}_i \right\} \\ {}
    \end{aligned}~~~\Leftrightarrow~~~\begin{aligned}
        \mathrm{minimize}~~&~~t \\
    ...
    \begin{equation}\begin{aligned}
        \mathrm{maximize}~~&~~r \\
        \mathrm{subject~to}~~&~~\mathbf{a}_i^{\top} \mathbf{x}_C + r\Vert\mathbf{a}_i\Vert_2\leq\mathbf{b}_i~~(i \in \mathbb{Z} \cap [1,m])
    \end{aligned}\end{equation}
\end{itemize}

\subsubsection*{Linear Fractional Programming}
\begin{equation}\begin{aligned}
    \mathrm{minimize}~~&~~\phi(\mathbf{x}) = \frac {\mathbf{c}^{\top}\mathbf{x} + d} {\mathbf{e}^{\top} \mathbf{x} + f} \\
    \mathrm{subject~to}~~&~~A\mathbf{x} \leq \mathbf{b} \\ {} \\ {}
\end{aligned}~~~\Leftrightarrow~~~\begin{aligned}
    \mathrm{minimize}~~&~~\mathbf{c}^{\top}\mathbf{y} + d\mathbf{z} \\
    \mathrm{subject~to}~~&~~A\mathbf{y} - b\mathbf{z} \leq 0 \\
        &~~\mathbf{e}^{\top}\mathbf{y} + f\mathbf{z} = 1\\
        &~~\mathbf{z} > 0
\end{aligned}\end{equation}
\begin{itemize}
    \item $\mathcal{\phi} = \left\{\mathbf{x}~|~\mathbf{e}^{\top}\mathbf{x} + f > 0 \right\}$
    \item Take $\mathbf{y} = \frac 1 {\mathbf{e}^{\top}\mathbf{x} + f}\mathbf{x}$, $z = \frac 1 {\mathbf{e}^{\top}\mathbf{x} + f}$
\end{itemize}
...
\begin{equation}\begin{aligned}
    \mathrm{minimize}~~&~~\frac 1 2 \mathbf{x}^{\top}P\mathbf{x} + \mathbf{q}^{\top}\mathbf{x} + r \\
    \mathrm{subject~to}~~&~~G\mathbf{x} \geq \mathbf{h} \\
        &~~A\mathbf{x} = \mathbf{b}
\end{aligned}\end{equation}
\begin{itemize}
...
\subsubsection*{Examples}
\begin{itemize}
    \item Unconstrained QP: $\mathrm{minimize}~~~~\frac 1 2 \mathbf{x}^{\top}P\mathbf{x} + \mathbf{q}^{\top}\mathbf{x} + r$
    \begin{itemize}
    ...
    \begin{itemize}
        \item $\mathbf{x}^{\ast} = \left(A^{\top}A\right)^{-1}A^{\top}\mathbf{b} = A^{\ast}\mathbf{b}$ ($A^{\ast}$: Left inverse of $A$ i.e. $A^{\ast}A = I$)
    \end{itemize}
    \item LP with Random Cost
    \begin{equation}\begin{aligned}
        \mathrm{minimize}~~&~~
            \mathbb{E}\left[ \mathbf{c}^{\top}\mathbf{x} \right] + \gamma \mathrm{Var}\left[ \mathbf{c}^{\top} \mathbf{x} \right]
            = \overline{\mathbf{c}}^{\top}\mathbf{x} + \gamma \mathbf{x}^{\top} \Sigma \mathbf{x} \\
    ...
    \begin{itemize}
        ...
        \item $\mathbf{c}^{\top}\mathbf{x}$: random variable with mean $\overline{\mathbf{c}}^{\top}\mathbf{x}$ and variance $\mathbf{x}^{\top}\Sigma\mathbf{x}$
...
\begin{equation}\begin{aligned}
    \mathrm{minimize}~~&~~\frac 1 2 \mathbf{x}^{\top}P_0\mathbf{x} + \mathbf{q}_0^{\top}\mathbf{x} + r_0 \\
    \mathrm{subject~to}~~&~~\frac 1 2 \mathbf{x}^{\top}P_i\mathbf{x} + \mathbf{q}_i^{\top}\mathbf{x} + r_i \leq 0~~(i \in \mathbb{Z} \cap [1,m]) \\
        &~~A\mathbf{x} = \mathbf{b}
\end{aligned}\end{equation}
\begin{itemize}
